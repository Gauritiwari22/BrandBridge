...
import { Sparkles, Loader2, IndianRupee, Users, Target, Calendar, Check, X, MessageSquare } from "lucide-react";
    ...
    <div className="mx-auto max-w-5xl p-8">
      <div className="rounded-2xl bg-hero-gradient p-8 text-white shadow-elegant">
        ...
        <div className="mt-6 flex flex-wrap gap-6 text-sm">
          <span className="flex items-center gap-2"><IndianRupee className="h-4 w-4" />{Number(campaign.budget).toLocaleString("en-IN")}</span>
            ...
            <Dialog>
              ...
              <DialogContent>
                ...
                <div className="space-y-3">
                  ...
                  <div><Label>Proposed rate (₹ INR)</Label><Input type="number" value={rate} onChange={e => setRate(e.target.value)} /></div>
            ...
            {applications.map(a => (
              <div key={a.id} className="flex items-start justify-between rounded-lg border border-border p-4">
                <div>
                  ...
                  {a.proposed_rate && <p className="mt-1 text-xs">Rate: {inr(a.proposed_rate)}</p>}
                </div>
                {a.status === "pending" && (
