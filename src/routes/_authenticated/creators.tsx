...
import { Instagram, Youtube, Twitter, Shield, MessageSquare } from "lucide-react";
...
function Creators() {
  ...
  const { data: creators, isLoading } = useQuery({
    queryKey: ["creators"],
    queryFn: async () => {
      const { data, error } = await supabase.from("profiles").select("*").in("role", ["influencer", "student", "business"]).order("trust_score", { ascending: false });
